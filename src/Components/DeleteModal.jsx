...
function DeleteModal({ itemId, active, onClose }) {
    ...
    // useEffect to handle which value has the active prop has ie project or worker or timings and set the url
    useEffect(() => {
        if (active === 'project') {
            setUrl('/projects/' + itemId)
        } else if (active === 'worker') {
            setUrl('/workers' + itemId)
        } else if (active === 'timings') {
            setUrl('/timings' + itemId)
        }
    }, [active])
