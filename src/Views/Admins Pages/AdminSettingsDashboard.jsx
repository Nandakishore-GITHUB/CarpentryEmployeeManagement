...
function AdminSettingsDashboard() {
    ...
    useEffect(() => {
        ...
        try {
            const data = requests.getData(URL)
            if (data.status === 200) {
                ...
                <div className="flex lg:w-60 md:w-48 lg:h-52 md:h-48 mx-6 md:my-10 -my-6 mb-5 relative">
                    <div className="md:bg-white flex items-center justify-center md:shadow-md w-full rounded-lg md:p-6 p-2 md:ml-0 ml-8">
                        <div className="flex md:flex-col gap-5">
                            <div>
                                <button className="text-gray-700 font-semibold md:text-2xl text-lg  bg-white rounded-lg lg:w-56 md:w-40 w-28 md:h-12 p-1 hover:text-violet-700 active:bg-violet-700 active:text-white" onClick={setURL('/project/all')}>Projects</button>
                            </div>
                            <div>
